...
public class S4MedianodTwoSortedArray {
...
//}
    public static double findMedianSortedArrays(int[] A, int[] B){
        ...
        if((m+n)%2 ==1) return findKth(A,0,m-1,B,0,n-1,(m+n)/2);
        else return (findKth(A,0,m-1,B,0,n-1,(m+n)/2) +findKth(A,0,m-1,B,0,n-1,(m+n)/2+1))/2.0;
    ...
    public static int findKth(int[] A,int al,int ar,int[] B, int bl, int br,int k){
        ...
        int alen = ar-al+1;
        int blen = br-bl+1;
        if(alen==0) return B[bl+k];//A的找完了
        if(blen==0) return A[al+k];//B的找完了
        if(k==0) return Math.min(A[al],B[bl]);
