...
//		mergeSort(nums,0,nums.length-1);
	 ...
	 * 二分插入(稳定)
	 * 时间复杂度：
	 * 思路：基本和直接插入一致，只是采用二分法在前面有序的序列中进行插入。
	 * 可以减少比较次数，但是移动次数和直接插入相同
	 * **/
	 ...
	 * 时间复杂度：O(nlogn)
	 * 
	 * 我的理解是：按照不同的增量进行分组，然后对每个组用直接插入排序
	 * 这里用的简单的增量序列d = {n/2 ,n/4, n/8 …..1}
	...
	/*简单选择排序(不稳定)
	 * 时间复杂度： O(n^2)
	 * 思想：每次从待排序的序列中选择最小的数字放到已经排序的表后面
	 * **/
	 ...
	 * 堆排序(不稳定)
	 * 时间复杂度O(nlogn)
	 * 性质：假设i节点是一个非端点节点，则parent(i)=1/2, leftChild(i)=2i,rightChild(i)=2*i+1
	 * 把初始的一位数组看做一个完全二叉树的一位存储方式
	 * 1、先把初始数组调整成最小堆，从n/2(取下界)的根节点开始调整最大堆
	 * 2、每次取H[0]和最后一个元素进行交换，即每次取当前堆顶放到后面
	 * 3、继续对0~i个元素进行调整
	 * 
	 * 树的深度是logn+1
	 * **/
	 ...
	 * 冒泡排序
	 * 时间复杂度：平均O(n^2)最好O(n)
	 * 改进：1、加入一个标志变量，某一次不发生交换时候就可以停止了。
...
//		}
	 ...
	 * 快速排序
	 * 时间复杂度O(nlogn)
	 * 当n较大时候用快排好，当序列基本有序时候用快排反而不好（会退化成为冒泡）
	 * **/
	//第一种，递归的方法
	...
	/*
	 * 快排改进：
	 * 快排被认为是同数量级O(nlogn)中平均性能最好的，但是如果初始序列按关键码有序或者基本有序的时候
	 * 快排反而退化成为冒泡排序。为改进，通常“三者取中”来选取基准记录。
	 * 即将排序区的两个端点与中间三个记录的居中的调整为支点记录。
	 * 
	 * 本改进算法是只对长度大于k的子序列调用快排，让其基本有序。
	 * 然后再对整个基本有序采用插入排序
	 * **/                                 
	/*
	 * 归并算法(稳定)
	...
	public static void merge(int[] a, int left, int middle, int right){
		...
		//将剩下的放到临时数组
		while(i<=middle && k<=right) tmpa[k++] = a[i++];
		while(j<=right && k<=right) tmpa[k++] = a[j++];
		//将临时数组复制回原来数组
		int t=left;
		while(t<=right){
			a[t]=tmpa[t];
